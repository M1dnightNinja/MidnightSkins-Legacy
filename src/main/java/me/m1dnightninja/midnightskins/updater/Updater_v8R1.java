...
public class Updater_v8R1 implements Updater {
    ...
    public Updater_v8R1() {
        ...
        Class<?> playerInfoData = ReflectionUtil.getNMSClass("PlayerInfoData");
        ...
        x = ReflectionUtil.getField(namedEntitySpawnPacket, "c");
        y = ReflectionUtil.getField(namedEntitySpawnPacket, "d");
        z = ReflectionUtil.getField(namedEntitySpawnPacket, "e");
        yaw = ReflectionUtil.getField(namedEntitySpawnPacket, "f");
        pitch = ReflectionUtil.getField(namedEntitySpawnPacket, "g");
    ...
    public void updatePlayerOther(Player p, Player o, String oldName) {
        ...
        if (p.getUniqueId() != o.getUniqueId() && o.getWorld().equals(p.getWorld())) {
            ...
            Object spawnPacket = ReflectionUtil.construct(ReflectionUtil.getConstructor(namedEntitySpawnPacket, entityHuman), ep);

            ReflectionUtil.setFieldValue(spawnPacket, x, p.getLocation().getBlockX());
            ReflectionUtil.setFieldValue(spawnPacket, y, p.getLocation().getBlockY());
            ReflectionUtil.setFieldValue(spawnPacket, z, p.getLocation().getBlockZ());
            ReflectionUtil.setFieldValue(spawnPacket, yaw, (byte) p.getLocation().getYaw());
            ReflectionUtil.setFieldValue(spawnPacket, pitch, (byte) p.getLocation().getPitch());

            PacketUtil.sendPacket(o, spawnPacket);
